...
class NextendSocialUser {
    ...
    protected function register($providerID, $email) {
        ...
        $username = strtolower($this->getAuthUserData('first_name') . $this->getAuthUserData('last_name'));
        if (empty($username)) {
            $username = strtolower($this->getAuthUserData('name'));
        }
        ...
        if (empty($username) || !validate_username($sanitized_user_login)) {
            ...
            $sanitized_user_login = sanitize_user($this->provider->settings->get('user_fallback') . $providerID, true);
        }
    ...
    public function registerComplete($user_id) {
        ...
        $this->provider->updateRedirectOnRegister();
